...
class FacekeypointClassify {
public:
    explicit FacekeypointClassify(const std::string &modelDir, const int cpuThreadNum,
...
private:
  ...
  void VisualizeResults(const std::vector<Face> &faces, cv::Mat *rgbaImage);
    ...
    // Visualize the results to origin image
  void VisualizeKeypointsResults(const std::vector<Face> &faces,
...
private:
  ...
  std::shared_ptr<FacekeypointClassify> faceClassifier_;
};

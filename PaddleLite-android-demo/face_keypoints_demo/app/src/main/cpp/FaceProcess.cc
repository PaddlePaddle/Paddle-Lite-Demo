...
cv::Mat rouge(cv::Mat image,  std::vector<cv::Point2d> points, bool ruby){
    // 自动涂口红
    cv::Mat image_cp;
    image.copyTo(image_cp);
    cv::Scalar rouge_color;
    if (ruby){
        rouge_color = cv::Scalar(0, 0, 255);
    }else{
        rouge_color = cv::Scalar(0, 0, 200);
    }
    // select points
    std::vector<cv::Point2d> lip;
    std::vector<int> hull;
    for (int k = 48; k < points.size(); k++){
        lip.push_back(points[k]);
    }
    // contours 寻找其凸包
    LOGI("---convexHull, lip: %d", lip.size());
    cv::convexHull(cv::Mat(lip), hull, true);
    LOGI("---contours");
    std::vector<cv::Point2d> contours;
    for (auto val : hull){
        contours.push_back(lip[val]);
    }
    cv::drawContours(image, contours, -1, rouge_color, -1);
    LOGI("---drawContours: %d", hull.size());
    cv::addWeighted(image, 0.2, image_cp, 1-0.1, 0, image_cp);
    LOGI("---addWeighted: %d", hull.size());
    return image_cp;
}

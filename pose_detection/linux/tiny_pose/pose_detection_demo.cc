...
RESULT get_keypoints(const float *score_map,int num_joints){
  ...
  for (int i = x-SEARCH_SCOPE; i <= x+SEARCH_SCOPE; i++) {
    for (int j = y-SEARCH_SCOPE; j <= y+SEARCH_SCOPE; j++) {
        result.keypoints.push_back({(float)i, (float)j});
...
std::array<float, 2> find_center(RESULT result,float scale_x,float scale_y){
  ...
  return {(x/sum_scores+0.5f)*scale_x-0.5f,(y/sum_scores+0.5f)*scale_y-0.5f};
}
...
cv::Mat process(cv::Mat &input_image, 
  ...
  double preprocess_time =(preprocess_end_time - preprocess_start_time) / 1000.f;
  ...
  //Run postprocess
  std::vector<RESULT> results =
  postprocess(output_data, output_size, SCORE_THRESHOLD,
...
int main(int argc, char **argv) {
  ...
  if (argc > 4) {
    ...
    while (1) {
      ...
      cap >> input_image;
      cv::Mat output_image = process(input_image, predictor);
      cv::imwrite("../../assets/images/posedet_demo_output"+std::to_string(coco)+".jpg", output_image);
      coco++;

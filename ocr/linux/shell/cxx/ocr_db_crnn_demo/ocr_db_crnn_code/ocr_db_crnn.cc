...
std::shared_ptr<PaddlePredictor> loadModel(std::string model_path) {
  MobileConfig config;
...
cv::Mat process(std::string det_model_file, std::string rec_model_file, std::string cls_model_file,
                std::string img_path, std::string output_img_path, std::string dict_path) { 
  ...
  //// visualization
  auto img_vis = Visualization(srcimg, boxes, output_img_path);
...
int main(int argc, char **argv) {
  if (argc < 6) {
    std::cerr << "[ERROR] usage: "
              << " ./ocr_db_crnn_demo det_model_file cls_model_file rec_model_file image_path"
                 " output_image_path charactor_dict\n";
    exit(1);
  }
  ...
  cv::Mat res_img = process(det_model_file, rec_model_file, cls_model_file, img_path, output_img_path, dict_path);
  return 0;
}
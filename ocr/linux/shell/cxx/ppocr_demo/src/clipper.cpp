...
void GetHorzDirection(TEdge& HorzEdge,
                      Direction& Dir,
                      cInt& Left,
                      cInt& Right) {
    ...
    if (IsIntermediate(e, topY)) {
      ...
      if (ePrev && ePrev->Curr.X == e->Bot.X && ePrev->Curr.Y == e->Bot.Y &&
        ...
        AddJoin(op, op2, e->Top);
                 ...
                 eNext->Curr.Y > eNext->Top.Y && SlopesEqual(e->Curr,
                                                             e->Top,
                                                             eNext->Curr,
                                                             eNext->Top,
                                                             m_UseFullRange) &&
...
void ClipperOffset::AddPath(const Path& path,
                            JoinType joinType,
                            EndType endType) {
...
void ClipperOffset::AddPaths(const Paths& paths,
                             JoinType joinType,
                             EndType endType) {
...
double DistanceFromLineSqrd(const IntPoint& pt,
                            const IntPoint& ln1,
                            const IntPoint& ln2) {
...
void AddPolyNodeToPaths(const PolyNode& polynode,
                        NodeType nodetype,
                        Paths& paths) {
  bool match = true;

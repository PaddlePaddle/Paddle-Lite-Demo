...
* 准备 ARMLiunx 开发版，将系统刷为 Ubuntu，用于 Demo 编译和运行。
...
* Paddle Lite 当前已验证 Khadas VIM3 （芯片为 Amlogic A311d）开发板，其它平台用户可自行尝试；
* 支持 usb camera
...
1. 目标检测 Demo 位于 `Paddle-Lite-Demo/object_detection/linux/picodet_detection` 目录
...
sh run.sh                   # 3. 执行物体检测（picodet 模型） demo，会直接启动图形界面并呈现检测结果。
```

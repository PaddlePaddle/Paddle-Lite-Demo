...
def scalar(value, tag='', anchor='', anchor_id=0):
    ...
    emit += ['"%s"' % encode(value)]
    handle += [
        'OnScalar(_, "%s", %s, "%s")' % (out_tag, anchor_id, encode(value))
    ]
...
def seq_start(tag='', anchor='', anchor_id=0, style='_'):
    ...
    emit += ['BeginSeq']
    handle += [
        'OnSequenceStart(_, "%s", %s, %s)' % (out_tag, anchor_id, style)
    ]
    return {'emit': emit, 'handle': handle}
...
def gen_templates():
    ...
    yield [[doc_start(), doc_start(True)], [seq_start()], [
        [], [scalar('foo')], [scalar('foo', 'tag')],
        [scalar('foo', '', 'anchor', 1)], [scalar('foo', 'tag', 'anchor', 1)],
        [scalar('foo'), scalar('bar')],
        [scalar('foo', 'tag', 'anchor', 1), scalar('bar', 'tag', 'other', 2)]
    ], [seq_end()], [doc_end(), doc_end(True)]]
    yield [[doc_start(), doc_start(True)], [map_start()], [
        [], [scalar('foo'), scalar('bar')],
        [scalar('foo', 'tag', 'anchor', 1), scalar('bar', 'tag', 'other', 2)]
    ], [map_end()], [doc_end(), doc_end(True)]]
    yield [[doc_start(True)], [map_start()],
           [[scalar('foo')], [seq_start(), scalar('foo'), seq_end()],
            [map_start(), scalar('foo'), scalar('bar'), map_end()]],
           [[scalar('foo')], [seq_start(), scalar('foo'), seq_end()],
            [map_start(), scalar('foo'), scalar('bar'), map_end()]],
           [map_end()], [doc_end(True)]]
    yield [[doc_start(True)], [seq_start()],
           [[scalar('foo')], [seq_start(), scalar('foo'), seq_end()],
            [map_start(), scalar('foo'), scalar('bar'), map_end()]],
           [[scalar('foo')], [seq_start(), scalar('foo'), seq_end()],
            [map_start(), scalar('foo'), scalar('bar'), map_end()]],
           [seq_end()], [doc_end(True)]]
...
def gen_tests():
    for events in gen_events():
        name = 'test' + hashlib.sha1(''.join(
            yaml.dump(event) for event in events)).hexdigest()[:20]
...
def create_emitter_tests(out):
    ...
    with Scope(out, 'namespace YAML', 0) as _:
        with Scope(out, 'namespace', 0) as _:
            ...
            for test in tests:
                with Scope(out, 'TEST_F(%s, %s)' %
                           ('GenEmitterTest', test['name']), 2) as _:

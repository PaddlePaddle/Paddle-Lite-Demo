...
void pre_process(std::shared_ptr<PaddlePredictor> predictor, const cv::Mat rgbaImage,
...
// Set the data of input image
  ...
  NHWC3ToNC3HW(reinterpret_cast<const float *>(resizedRGBImage.data), inputData,
               inputMean.data(), inputStd.data(), inputShape[3],
               inputShape[2]);
...
void post_process(std::shared_ptr<PaddlePredictor> predictor, std::vector<RESULT> *results,float scoreThreshold, int width, int height, std::vector<std::string> labelList,
...
void VisualizeResults(const std::vector<RESULT> &results, cv::Mat *rgbaImage) {
  ...
  for (int i = 0; i < results.size(); i++) {
    ...
    std::cout << "detection, image size: " << w << ", "
     << h << ", detect object: " << object.class_name 
     << ", score: " << object.score << ", location: x=" << boundingBox.x
     << ", y=" << boundingBox.y << ", width=" << boundingBox.width << ", height=" << boundingBox.height
     << std::endl;
  }
}
...
void run_model(std::string model_file, std::string img_path,
  ...
  std::string result_name = img_name + "_yolo_v3_mobilenetv3_detection_result.jpg";
  cv::imwrite(result_name, img);
  std::cout << "result has been saved to picture: "<<result_name<<" "<<std::endl;
}

...
#endif
/*
 * Class:     Java_com_baidu_paddle_lite_demo_segmentation_Native
 * Method:    nativeInit
 * Signature:
 * (Ljava/lang/String;Ljava/lang/String;ILjava/lang/String;II[F[FF)J
 */
JNIEXPORT jlong JNICALL
Java_com_baidu_paddle_lite_demo_segmentation_Native_nativeInit(
...
Java_com_baidu_paddle_lite_demo_segmentation_Native_nativeRelease(JNIEnv *env,
                                                                  jclass thiz,
                                                                  jlong ctx) {
  if (ctx == 0) {
    return JNI_FALSE;
...
Java_com_baidu_paddle_lite_demo_segmentation_Native_nativeProcess(
  ...
  cv::Mat out_cv_img;
  pipeline->GetOutImg(&out_cv_img);

  cv::cvtColor(out_cv_img, out_cv_img, cv::COLOR_RGB2RGBA);
  if (AndroidBitmap_getInfo(env, OutBitmap, &bitmapInfo) < 0) {
    LOGE("Invoke AndroidBitmap_getInfo() failed!");
    return JNI_FALSE;
  }
  cv::resize(out_cv_img, out_cv_img,
             cv::Size(bitmapInfo.width, bitmapInfo.height), 0.f, 0.f);

  bool modified = res_str.empty();
  if (!modified) {
    // Convert the OpenCV RGBA image to the android bitmap(ARGB8888)
    if (out_cv_img.type() != CV_8UC4) {
    ...
    cv::Mat bmpImage(bitmapInfo.height, bitmapInfo.width, CV_8UC4,
                     bitmapPixels);
    out_cv_img.copyTo(bmpImage);
    if (AndroidBitmap_unlockPixels(env, OutBitmap) < 0) {
      LOGE("Invoke AndroidBitmap_unlockPixels() failed!");
      return JNI_FALSE;
    }

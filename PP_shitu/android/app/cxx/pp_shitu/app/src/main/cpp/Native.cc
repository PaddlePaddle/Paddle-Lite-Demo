...
Java_com_baidu_paddle_lite_demo_pp_1shitu_Native_nativeInit(
    ...
    jstring jLabelPath, jlongArray jDetInputShape, jlongArray jRecInputShape,
    jint cpuThreadNum, jint WarmUp, jint Repeats, jint topk,
    jstring cpu_power) {
  ...
  return reinterpret_cast<jlong>(new PipeLine(
      det_model_path, rec_model_path, label_path, det_input_shape_int,
      rec_input_shape_int, cpuThreadNum, WarmUp, Repeats, topk, cpu_mode));
}
...
Java_com_baidu_paddle_lite_demo_pp_1shitu_Native_nativeProcess(
  ...
  std::string res_str = pipeline->run(input_mat, out_object, 1);

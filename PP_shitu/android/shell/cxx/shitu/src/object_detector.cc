...
void ObjectDetector::Predict(const std::vector<cv::Mat> &imgs,
  ...
  auto preprocess_start = GetCurrentUS();
  // Preprocess image
  ...
  auto preprocess_end = GetCurrentUS();
  ...
  for (const auto &tensor_name : input_names) {
    ...
    if (tensor_name == "image") {
      ...
      in_tensor->Resize({batch_size, det_input_shape_[1], det_input_shape_[2],
                         det_input_shape_[3]});
  ...
  bool is_rbox = false;
  auto inference_start = GetCurrentUS();
  for (int i = 0; i < repeat_; i++) {
    predictor_->Run();
  }
  auto inference_end = GetCurrentUS();
  auto postprocess_start = GetCurrentUS();
  // Get output tensor
  ...
  result->clear();
  if (arch_ == "PicoDet") {
    PPShiTu::PicoDetPostProcess(result, output_data_list_, fpn_stride_,
                                inputs_.im_shape_, inputs_.scale_factor_,
                                score_threshold_, nms_threshold_, num_class,
                                reg_max);
  ...
  auto postprocess_end = GetCurrentUS();
  times->push_back((preprocess_end - preprocess_start) / 1000.f);
  times->push_back((inference_end - inference_start) / 1000.f / repeat_);
  times->push_back((postprocess_end - postprocess_start) / 1000.f);
}
} // NOLINT
